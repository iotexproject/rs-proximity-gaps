...
/-- **Batch CA union bound**: a union of m singletons has cardinality ≤ m.
    The mathematical content is that each coordinate contributes at most
    1 bad coefficient value (by batch_ca_at_most_one), so the total
    number of bad coefficient tuples is at most m, giving probability m/|F|. -/
