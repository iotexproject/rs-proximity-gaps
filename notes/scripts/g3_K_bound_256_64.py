"""g3_K_bound_256_64.py — push to (256, 64) for strongest universal claim.

Test 2-3 Reverse Pattern supports at (256, 64), q = 257 (smallest q ≡ 1 mod 256).

Note: (256, 64) has L_0 order 256, L_1 order 128, L_2 order 64.
At rate 1/4: M_max(L_1) = 128 - 64 + 1 = 65. M_max(L_2) = 64 - 32 + 1 = 33.

If K ≤ 10 still: TRULY universal (not just at L_2 ≤ 32).
"""
...
def main():
    p = 257  # smallest q ≡ 1 mod 256 (need 256 | q-1)
    n0, k0, R = 256, 64, 2
    n1, k1 = 128, 32
    n2, k2 = 64, 16
    w_J_L0, w_J_L1, w_J_L2 = 128, 64, 32

    print(f"=== K bound at (256, 64), q={p} ===")
    print(f"  L_0 order {n0}, L_1 order {n1}, L_2 order {n2}")
    print(f"  w_J: L_0={w_J_L0}, L_1={w_J_L1}, L_2={w_J_L2}")
    print(f"  M_max(L_1)=65, M_max(L_2)=33")
    print(f"  Test: K ≤ 10 (universal) vs larger?\n")
    ...
    print(f"  Sampled info_sets at L_2: {len(info_sets_n2)}")
    ...
    while len(info_sample_n0) < 20000:
    ...
    # Try a "scaled-up" Reverse support — e.g. (175, 198, 203)
    test_supports = [
        (175, 198, 203), # mod-4 = (3, 2, 3) Reverse, large positions
        (171, 174, 175), # mod-4 = (3, 2, 3) Reverse
    ]
    ...
    for sup in test_supports:
        ...
        if d_f <= w_J_L0:
            print(f"  SKIP: not above-J at L_0"); continue
        ...
        if d_fold_max <= w_J_L1:
            print(f"  SKIP: not above-J at L_1"); continue
        ...
        for a1 in range(p):
            ...
            if (a1 + 1) % 50 == 0:
                ...
                print(f"    [α_1={a1+1}/{p}, elapsed={elapsed:.0f}s]")
        elapsed = time.time() - t0
