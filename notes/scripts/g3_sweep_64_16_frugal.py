"""g3_sweep_64_16_frugal.py — memory-frugal sweep at (64, 16) hunting for
strict-above-J supports with count > 17 = n_1 - s_1 + 1 at (64, 16).

Memory budget per worker: < 1GB. Use 4 workers (not 20).

Strategy:
  1. Generate 100 random 3-pos supports in syndrome window {16..63} \\ {32..47}.
     (Exclude {32..47} where count = q is forced.)
  2. For each, sample 5000 info sets at L_0 (for d_0 above-J check).
  3. If above-J: sample 5000 info sets at L_1 and count bad α via small q = 257.
"""
...
def worker_init(p, n0, k0, n_samples_n0, n_samples_n1):
    ...
    # Sampled info sets at L_0 and L_1
    ...
    _state.update({
        ...
        'w_J': n0 - int(math.isqrt(k0 * n0)),  # 64 - 32 = 32
        'threshold_d1': n1 - int(math.isqrt(k1 * n1)),  # 32 - 16 = 16
    })
...
def worker_check(positions_coeffs):
    ...
    d_f = n0 - k0 - max_extras  # upper bound on true d_0
    if d_f <= w_J:
        return (positions, False, d_f, None, None)
    ...
    for a in range(p):
        ...
        if d1 <= threshold:
            ...
            if cnt <= 25:
                bad.append(a)
    return (positions, True, d_f, cnt, bad)


def generate_candidates(p, n0, k0, n_candidates, seed):
    rng = random.Random(seed)
    out = []
    target_window = set(range(n0 // 2, n0 // 2 + n0 // 4))  # {32..47} for (64,16)
    while len(out) < n_candidates:
        n_pos = rng.choice([3, 4, 5])
        positions = sorted(rng.sample(range(k0, n0), n_pos))
        if set(positions) <= target_window:
            continue  # skip — would be count=q (at-Johnson)
...
def main():
    ...
    n0, k0 = 64, 16
    n_candidates = 200
    n_workers = 8  # studio: 28 cores, plenty headroom
    n_samples_n0 = 5000
    n_samples_n1 = 5000
    seed = 2026 + p

    print(f"=== Memory-frugal (64,16) sweep at q={p}, {n_candidates} cands, {n_workers} workers ===")
    ...
    print(f"  PR #373 single-line bound: count ≤ n_1 - s + 1 = {n0//2 - int(math.isqrt((k0//2)*(n0//2))) + 1}")
    ...
    with Pool(processes=n_workers, initializer=worker_init,
              initargs=(p, n0, k0, n_samples_n0, n_samples_n1)) as pool:
        for i, res in enumerate(pool.imap_unordered(worker_check, candidates, chunksize=2)):
            if res[1]:
                ...
                if cnt > max_count:
                    max_count = cnt
                    print(f"  [{i+1}/{len(candidates)}] above-J d_f={d_f} count={cnt} pos={res[0]} bad={bad[:10]}", flush=True)
            else:
                n_below += 1
            if (i+1) % 10 == 0:
